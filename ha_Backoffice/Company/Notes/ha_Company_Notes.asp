...
End If
	...
	Call CheckBoxRead(1, "Notes - Category")
%>
        ...
        <table class="MainTable">
		  ...
		  <tr valign="top">
		    <td width="16%">
  		    ...
  		    <td width="84%">
			  <div class="ListBody">
				<%'=TopRightLinksExpanded("", "", "", "", "", "", "", "", "", "Add Note")%>
		...
		<!--#include virtual="ha_backoffice/includes/FunctionsPageStart.inc"--> 
				  ...
				  <tr id="row[<%=row%>]" style="background-color: #FBFBFB;">
					...
					<td class="row" style="<%=detailStyle%>">
					  <% response.write iif(len(rs("Title")) > 21, mid(rs("Title"),1,20) & "...",rs("Title")) %>
					</td>
					
					<td class="row" style="<%=detailStyle%>">
					  <% response.write iif(len(rs("Note")) > 41, mid(rs("Note"),1,40) & "...",rs("Note")) %>
					</td>
					...
					<%if Session("ha_shortname") = rs("CreatedBy") then%>
					  ...
					  <a href="javascript:clickNoteEdit2(<%=rs("ID")%>,'<%=Replace(Replace(strNote,"'","\'"),chr(34),"&quot;")%>','<%=rs("Category")%>');">Edit</a>
					<%else%>
					<a href="javascript:clickViewNote(<%=rs("ID")%>,'<%=Replace(Replace(strNote,"'","\'"),chr(34),"&quot;")%>');">View</a>
					<%End if%>
					</td>
				  </tr>
