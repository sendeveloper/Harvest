...
<%
    Dim rs, rsSec, strSQL, ID, rsNotes
...
Set rs = Server.CreateObject("ADODB.Recordset")
	...
	if request.querystring("Mode") = "Save" or Request.QueryString("Mode") = "Edit" then
	  ...
	  <tr>
		<td colspan="2">
		  <table width="100%" cellspacing="0" cellpadding="0">
			...
			<tr>
			  <td colspan="2">
			  <div id="divNotesContainer" style="scroll:auto;overflow-y:scroll;height:210px;">
				<table width="100%" cellspacing="0" cellpadding="0">
					  ...
					  <table width='100%' cellspacing='0' cellpadding='0'>
						...
						<tr valign="top">
						  <td width="100%">
							...
							<%SET rsNotes = NotesRead2(ID, "ToDo", 230)%>
							<table width="99%" border="0" cellspacing="0" cellpadding="0">
							<%while not rsNotes.eof%>
		<%
			dim m,d,y
			m = right("0" & DatePart("m",rsNotes("createdate")),2)
			d = right("0" & DatePart("d",rsNotes("createdate")),2)
			y = Datepart("yyyy",rsNotes("createdate"))
		%>
      <tr style="background-color:lightgrey;line-height:16px;">
	  <td  width="12%">
        <div style="position:relative;float:left;margin-top:5px;">
		<table width="100%">
		<tr><td>Date:</td><td style="text-align:right;"><%Response.Write m & "-" & d & "-" & y %></td></tr>
		<tr><td>Time:</td><td style="float:right;"><%=MID(rsNotes("createtime"),10,10)%></td></tr>
		<tr><td>User:</td><td><%=rsNotes("username")%></td></tr>
		</table>
         
      </div></td>
      <td  width="80%">
	  <div id='note<%=rsNotes("ID")%>' name='note<%=rsNotes("ID")%>' style="margin-top: 4px; height:60px; background-color: #FFFFFF;vertical-align: top;text-align: left;overflow-y: scroll;float:center;"><%=rsNotes("result")%></div>
	  </td>
      <td   width="5%">
		 
		 <%if datediff("h",rsNotes("createdate"),now) < 24 then%>
		 <br/>
		 <br/>
		 <br/>
				&nbsp;<a href="javascript:clickNoteEdit(<%=rsNotes("ID")%>);" style="vertical-align:bottom;" class="bo_Button40">Edit</a>
       <%end if%>
		</td>
