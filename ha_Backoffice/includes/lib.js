...
function log(e) {
  ...
  else if (console instanceof Array) {console.push(e);}
  else if (console instanceof Function) {console(e);}
  else if (console instanceof Object) {
    console.appendChild(text(inspect(e)));
    console.appendChild(node("br"));}
  else {alert("Log to invalid console");}}
log.clear = function clear() {
  if (!console) {return false;}
  else if (console instanceof Array) {console.length = 0; return console;}
  else if (console instanceof Function) {if (clear in console && console.clear instanceof Function) {return console.clear();}}
  else if (console instanceof Object) {console.innerHTML = ""; return console;}
  else {alert("Log to invalid console");}}
...
function inspect(e) {
  ...
  else if (e instanceof String) {return "\"" + e + "\"";}
  else if (e instanceof Number) {return e.toString();}
  else if (e instanceof Array) {return "[" + e.forEach(inspect).join(", ") + "]";}
...
// create (nested) nodes -- children are HTML nodes or lists of HTML nodes; attributes are js-objects
...
// obsolete version remains for backwards-compatibility; attributes is a plist-style array
...
function showhide(node) {
  return node.style.visibility = (node.style.visibility === "hidden") ? "show" : "hidden";}
...
function help(helpId) {
  var win = ajax("https://www.zip2tax.info/z2t_BackOffice/Research/z2t_ResearchOperations_operate.asp?op=" + helpId, function(body){this.result.children[0].children[0].innerHTML = body; this.result.style.visibility = "visible"; return;});
...
function ajaxChange(state) {
  try {
    ...
    case 4:
      ...
      switch (this.status) {
      case 200:
        this.fn.call(this, this.responseText, this.responseXML);
        break;
      default:
        this.error.call(this, this.responseText, this.responseXML);
        return;
        break;}
      break;
    default:
