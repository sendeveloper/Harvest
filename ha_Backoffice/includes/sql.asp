...
sqlConn.Open "Driver=SQL Server;server=66.119.55.118,7043;uid=davewj2o;pwd=get2it;database=ha_backoffice" 'casper10
...
' Return the string representation of the resultsets in rs
Function rsTable(rs)
  If RowMod = 0 Then RowMod = 1
  Dim table
  table = ""
''''  Dim rsTryNext
...
'        Response.Write(TypeName(field))
'        table = table & field
        Select Case TypeName(field)
        Case "Field"
          If field.Type = 12 Then
            table = table & rsTable(field)
          Else
            table = table & "-" & field.Type & "-"
          End If
        Case "Variant"
        Case Else
          table = table & typename(fieldrr)
        End Select
        
        table = table & "</td>" & chr(13)
      Next
      table = table & "      </tr>" & chr(13)
      rs.moveNext
    Loop
