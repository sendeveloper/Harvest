...
<%
	...
	If isnull(Request("aID")) or Request("aID") = "" Then
		AccountID = ""
		TaskName = "TableDistribution_ha_Accounts_update (Full)"
	Else
		AccountID = Request("aID")
		TaskName = "TableDistribution_ha_Accounts_update (Partial)"
	End If
	...
	Else
		...
		If AccountID = "" Then			
			SQL = "ha_TableDistribution_ha_Accounts_update('" & RequestDatabase & "', " & _
				"0, " & RequestStart & ", " & RequestEnd & ")"
		Else
			SQL = "ha_TableDistribution_ha_Accounts_update('" & RequestDatabase & "', " & AccountID & ")"
		End If		
	...
	'Post to Task Log
	Task = RequestExecBy & " --> ha_Accounts Update: " & RequestServer & " " & RequestDatabase
	If AccountID = "" Then
