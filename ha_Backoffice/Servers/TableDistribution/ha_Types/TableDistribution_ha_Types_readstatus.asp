...
<%
    ...
    response.write "<ha_Types_status>"
	...
	Else
		...
		If UCase(Left(RequestServer,6)) = "PHILLY" Then
			...
			SQL = "ha_Types_status('" & RequestServer & "', '" & RequestDatabase & "')"
		Else
			ConnString = "driver=SQL Server;server=127.0.0.1,7043;uid=davewj2o;pwd=get2it;database=ha_Processes"
			conn.Open ConnString
			SQL = "TableDistribution_ha_Types_status('" & RequestServer & "', '" & RequestDatabase & "')"
	...
	'Post to Task Log
	Task = ExecBy & " --> ha_Types_Status: " & RequestServer & " " & RequestDatabase
