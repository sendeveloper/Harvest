...
<script language='JavaScript'>
  ...
  var updateFlag = 0;
    ...
    function runStatusCheck()
        {
		var url = '<%=strPathServers%>TableDistribution/z2t_Types/TableDistribution_z2t_Types_readstatus.asp' +
    ...
    function getResponse()
      ...
      if (http.readyState == 4)
        ...
        if (http.status == 200)
          ...
          div.style.borderBottom = div.style.borderTop = "1px solid black;";
          document.getElementsByTagName("body")[0].appendChild(div);
          //div.appendChild(http.responseXML);
  ...
  function runUpdate()
        {
		var url = '<%=strPathServers%>TableDistribution/z2t_Types/TableDistribution_z2t_Types_update.asp' +
			'?s=' + locationServer[loopStep] +
			'&d=' + locationDatabase[loopStep] +
			'&Now=' + escape(Date());
		//alert(url);
