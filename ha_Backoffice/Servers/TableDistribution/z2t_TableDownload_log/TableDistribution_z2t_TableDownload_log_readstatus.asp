...
<%
    ...
    response.write "<z2t_TableDownload_log_status>"
	...
	Else
		...
		If UCase(Left(RequestServer,6)) = "PHILLY" THEN
			SQL = "z2t_TableDownload_log_status('" & RequestServer & "', '" & RequestDatabase & "')"
		Else	
			SQL = "z2t_TableDownload_log_status"
		End If
		...
		If not rs.eof then
			...
			response.write "<response_table_name>z2t_TableDownload_log</response_table_name>"
	...
	'Post to Task Log
	Task = ExecBy & " --> z2t TableDownload_log Status: " & RequestServer & " " & RequestDatabase
