...
<script language='JavaScript'>
	...
	function runUpdate() 
	{
        ...
        var successFn16 = function(ignore, r, timer) 
        {
            clearTimeout(timer);
            getUpdateResponse(ignore, r, 16);
        };

        var errorFn16 = function(r, ignore, timer) 
		{
            clearTimeout(timer);
            errorfn(r, ignore, 16);
        };

        var timeoutFn16 = function(ignore, ignore2, timer) 
        {
            clearTimeout(timer);
            linkToError("ajax timeout", "Timed out with no response.")(document.querySelectorAll("#resultUpdated" + 16)[0].firstChild);
        }
        ...
        http[16] = xhr(getURL(16), successFn16, errorFn16, 15000, timeoutFn16);
        errorFn16.timer = http.timer;
        successFn16 = http.timer;
        http[16].get(); 
        // http.open('GET', getURL(1), true);
