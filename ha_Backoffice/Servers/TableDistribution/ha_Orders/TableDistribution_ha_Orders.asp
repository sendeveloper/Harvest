...
<script language='JavaScript'>
    ...
    function runStatusCheck() 
        {		
		var url = '<%=strPathServers%>TableDistribution/ha_Orders/TableDistribution_ha_Orders_readstatus.asp' +
    ...
    function getResponse() 
        ...
        if (http.readyState == 4) 
			...
			if (http.status == 200) 
				...
				if (loopStep == 0)
					{
					aRecMax = r.getElementsByTagName('response_maximum_OrderID')[0].firstChild.nodeValue;
					}
	...
	function runUpdate() 
        {		
		var url = '<%=strPathServers%>TableDistribution/ha_Orders/TableDistribution_ha_Orders_update.asp' +
			'?s=' + locationServer[loopStep] +
			'&d=' + locationDatabase[loopStep] +
			'&Start=' + aRecStart +
			'&End=' + aRecEnd +
			'&ExecBy=TableDistribution_ha_Orders.asp' +
			'&Now=' + escape(Date());
		//alert(url);
