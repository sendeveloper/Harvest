...
<%
	...
	If isnull(Request("oID")) or Request("oID") = "" Then
		OrderID = 0
		TaskName = "TableDistribution_ha_Orders_update (Full)"
	Else
		OrderID = Request("oID")
		TaskName = "TableDistribution_ha_Orders_update (Partial)"
	End If
	...
	If OrderID = 0 Then
		SQL = "ha_Orders_repl_Table_refresh(0, " & RequestStart & ", " & RequestEnd & ")"
	Else
		SQL = "ha_Orders_repl_Table_refresh(" & OrderID & ")"
	End If
	...
	response.write "<request_variable_OrderID>" & OrderID & "</request_variable_OrderID>"
	...
	'Post to Task Log
	Task = ExecBy & " --> ha_Orders_Update: " & RequestServer & " " & RequestDatabase & _
		" OrderID=" & OrderID & " Start=" & RequestStart & " End=" & RequestEnd
