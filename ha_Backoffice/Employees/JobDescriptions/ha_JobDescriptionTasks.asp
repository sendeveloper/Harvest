...
<%
	...
	ColorTab = 4
	PageHeading = "Job Description Tasks"
	...
	Call CheckBoxRead(1, "JobDescriptions - TaskCategories")
...
<head>
  ...
  <script type="text/javascript">
	var delTable = 'ha_JobDescriptionTasks';
	<%=FunctionsFilter%>
	
    function clickAddJobDescriptionTask()
        {
        var URL = 'ha_JobDescriptionTasks_edit.asp' +
            '?ID=0';
        openPopUp(URL);
        }		
		
	function clickEditJobDescriptionTask(ID)
        {
        var URL = 'ha_JobDescriptionTasks_edit.asp' +
            '?ID=' + ID;
        openPopUp(URL);
        ...
        <table class="MainTable">
		  ...
		  <tr valign="top">
		    <td width="16%">
  		    ...
  		    <td width="84%">
			  <div class="ListBody">
				<%=TopRightLinksExpanded("", "", "", "", "", "", "", "", "", "Add Job Description Task")%>
...
<%
		...
		strSQL = "ha_JobDescriptionTasks_list('" & CheckBoxes(1,0) & "', '" & OrderBy & "', '" & Request("SearchText") & "')"
...
%>
		<!--#include virtual="ha_backoffice/includes/FunctionsPageStart.inc"--> 
				  ...
				  <tr id="row[<%=row%>]" style="background-color: #FBFBFB;">
					...
					<td class="row" style="<%=detailStyle%> text-align: center">
					  <a href="javascript:clickEditJobDescriptionTask(<%=rs("ID")%>)">Edit</a>
					  <a href="javascript:clickDelete(<%=rs("ID")%>, '<%=rs("TaskName")%>', <%=row%>);">Delete</a>
					</td>
				  </tr>
