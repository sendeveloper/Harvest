...
<%
    ...
    If Request("ID") = "" Then
		Title = "Add To Employee Email Aliases "
	Else
		Title = "Edit Employee Email Aliases"
...
<body onLoad="formLoad();">
  <form method="Post" action="ha_EmployeeEmailAliases_post.asp" name="frm">
    <input type="hidden" id="AliasID" name="AliasID" value="<%=ID%>">
...
<%
					strSQL = "ha_Employee_EmailAliases_Members_list(" & Request("ID") & ")"
					rs.Open strSQL, connLocal, 3, 3, 4
					If not rs.eof Then
						While not rs.eof
							Response.write "<tr class=""MembersTd"">"  & rs("HTMLAliasMember") &_
...
<%
		strSQL = "ha_Employee_EmailAlias_Members_credits(" & Request("ID") & ")"
		rs.Open strSQL, connLocal, 3, 3, 4
					If not rs.eof Then
