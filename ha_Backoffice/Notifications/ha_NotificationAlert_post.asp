...
<head>
  ...
  <title>Harvest American Backoffice - Post Alert</title>
</head>
...
<%
    ...
    If Request("ID") = "0" Then
      strSQL = "SELECT * FROM ha_NotificationAlerts"
      rs.Open strSQL, connCasper10, 2, 3
      rs.AddNew
	  rs("CreatedBy") = Session("ha_shortname")
      rs("CreatedDate") = Now
    Else
      strSQL = "SELECT * FROM ha_NotificationAlerts WHERE ID = " & Request("ID")
      rs.Open strSQL, connCasper10, 2, 3
    ...
    End If
		...
		rs("TaskLogSearchCriteriaTaskName") = 	request("TaskLogSearchCriteriaTaskName")
		rs("TaskLogSearchCriteriaTask")		=	request("TaskLogSearchCriteriaTask")			
		rs("TaskLogSearchCriteria")			=	request("TaskLogSearchCriteria")				
    rs.Update
    rs.Close
%>
